...
def test_selected_report_browser_context_api_becomes_observatory_ready_and_idempotent(
    tmp_path: Path,
) -> None:
    ...
    first = acquire_selected_report_core_sources_in_browser_context(
    ...
    assert first.api_request_count == 3
    ...
    assert all(result.source_mode == "browser_context_api" for result in first.results)
    ...
    with duckdb.connect(str(database), read_only=True) as connection:
        capture_modes = {
            ...
            for row in connection.execute(
                """
                SELECT DISTINCT capture_mode
                FROM source_acquisition_observation
                WHERE outcome = 'schema_candidate'
                """
            ).fetchall()
        }
    assert capture_modes == {"browser_context_api"}

    second = acquire_selected_report_core_sources_in_browser_context(
        database_path=database,
        migrations_path=root / "migrations",
        raw_root=raw_root,
        registry_path=root / "config" / "ascension_logs_sources.yaml",
    ...
    assert summary["browser_context_api_used_this_run"] is True
    assert summary["browser_navigation_performed"] is True
    assert summary["browser_har_used_this_run"] is False
    assert summary["browser_trace_used_this_run"] is False
    assert summary["direct_http_used_this_run"] is False
