...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build a scalar-safe static field-lineage review from a local pinned Lua source tree. "
            "The command performs no network requests and does not infer backend or "
            "gameplay semantics."
        )
    )
    parser.add_argument(
...
def main() -> int:
    ...
    if args.output:
        write_lua_lineage_review(args.output, review)
        print(args.output)
    else:
