...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build one bounded boss/location/difficulty population context from the official "
            "public statistics API. Existing provenance-complete slices are reused."
        )
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/exchange/out/coa-public-api-encounter-context-review.json"),
    )
    ...
    before = review_public_api_encounter_context(
        args.database,
        args.migrations,
        phase=phase_number,
        ...
        for index in before.missing_slice_indexes:
            ...
            if capture.query_value_mapping() != _query_text(query):
                raise SystemExit(
                    ...
                    "encounter profile; refusing to persist it."
                )
    ...
    after = review_public_api_encounter_context(
        args.database,
        args.migrations,
        phase=phase_number,
        difficulty=args.difficulty,
    ...
    receipt = {
        "schema_version": 1,
        "review_kind": "official_public_api_encounter_population_context",
        "encounter_binding": {
            "reference_url_validated": True,
            "boss_catalog_unique_match": True,
            "boss_scope_bound": True,
            "location_scope_bound": True,
            "difficulty_scope_bound": True,
            "phase_scope_bound": True,
        ...
        "verification": {
            "concrete_encounter_reference_validated": True,
            "official_boss_catalog_binding_used": True,
            "encounter_population_context_complete": after.complete,
